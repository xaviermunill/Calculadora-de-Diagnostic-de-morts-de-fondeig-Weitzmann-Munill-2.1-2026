...
const createDefaultBlockItem = (
  ...
  return {
    ...
    c2_abrasionAreaM2: baseRecord?.c2_abrasionAreaM2 ?? baseRecord?.c2_haloRadiusM ?? 5,
    c2_haloRadiusM: baseRecord?.c2_abrasionAreaM2 ?? baseRecord?.c2_haloRadiusM ?? 5,
    ...
    notes: '',
    evaluatorCriteria: baseRecord?.evaluatorCriteria || {
    ...
    seabedTypes: baseRecord?.seabedTypes ? [...baseRecord.seabedTypes] : [],
    posidoniaDistances: baseRecord?.posidoniaDistances ? [...baseRecord.posidoniaDistances] : [],
  };
};
...
export const Calculator: React.FC<CalculatorProps> = ({
  ...
  const [blocks, setBlocks] = useState<BlockItem[]>(() => {
    if (initialRecord?.blocks && initialRecord.blocks.length > 0) {
      return initialRecord.blocks.map((b, idx) => ({
        ...
        c2_abrasionAreaM2: b.c2_abrasionAreaM2 ?? b.c2_haloRadiusM ?? initialRecord.c2_abrasionAreaM2 ?? initialRecord.c2_haloRadiusM ?? 5,
        c2_haloRadiusM: b.c2_abrasionAreaM2 ?? b.c2_haloRadiusM ?? initialRecord.c2_abrasionAreaM2 ?? initialRecord.c2_haloRadiusM ?? 5,
        ...
        evaluatorCriteria: b.evaluatorCriteria || initialRecord.evaluatorCriteria,
        seabedTypes: b.seabedTypes || initialRecord.seabedTypes || [],
        posidoniaDistances: b.posidoniaDistances || initialRecord.posidoniaDistances || [],
        notes: b.notes || '',
      }));
    }
  ...
  const handleCopyFromBlock1 = () => {
    ...
    updateActiveBlock((current) => ({
      ...
      evaluatorCriteria: b1.evaluatorCriteria ? { ...b1.evaluatorCriteria } : undefined,
      seabedTypes: b1.seabedTypes ? [...b1.seabedTypes] : [],
      posidoniaDistances: b1.posidoniaDistances ? [...b1.posidoniaDistances] : [],
      notes: b1.notes,
    }));
  ...
  const handleApplyToAllBlocks = () => {
    ...
    setBlocks((prev) =>
      prev.map((b, idx) => ({
        ...
        c2_abrasionAreaM2: src.c2_abrasionAreaM2,
        c2_haloRadiusM: src.c2_abrasionAreaM2 ?? src.c2_haloRadiusM,
        ...
        evaluatorCriteria: src.evaluatorCriteria ? { ...src.evaluatorCriteria } : undefined,
        seabedTypes: src.seabedTypes ? [...src.seabedTypes] : [],
        posidoniaDistances: src.posidoniaDistances ? [...src.posidoniaDistances] : [],
  ...
  }, [blocks, depthM, presenceStatus, connectionMode, combinedPhysics, usageStatus]);
  ...
  const buildCurrentRecord = (): MortEvaluationRecord => {
    ...
    return {
      ...
      c2_abrasionAreaM2: firstBlock.c2_abrasionAreaM2 ?? firstBlock.c2_haloRadiusM,
      c2_haloRadiusM: firstBlock.c2_abrasionAreaM2 ?? firstBlock.c2_haloRadiusM,
      ...
      evaluatorCriteria: firstBlock.evaluatorCriteria || activeEvaluatorCriteria,
      seabedTypes: firstBlock.seabedTypes && firstBlock.seabedTypes.length > 0 ? firstBlock.seabedTypes : undefined,
      posidoniaDistances: firstBlock.posidoniaDistances && firstBlock.posidoniaDistances.length > 0 ? firstBlock.posidoniaDistances : undefined,
  ...
  const activeC2AbrasionArea = rawActiveBlock.c2_abrasionAreaM2 ?? rawActiveBlock.c2_haloRadiusM ?? 5;
  ...
  // Active block Evaluator Criteria
  const activeEvaluatorCriteria: EvaluatorCriteria = rawActiveBlock.evaluatorCriteria || {
    absencePosidoniaOrHabitats: false,
    immediateRemoval: false,
    scheduledRemoval: false,
    neutralizeAndMaintain: false,
  ...
  const activeEvaluatorCriteriaLabels = getActiveEvaluatorCriteriaLabels(activeEvaluatorCriteria);
  const activeBlockEvaluatorCriteriaCount = activeEvaluatorCriteriaLabels.length;

  const toggleActiveEvaluatorCriteria = (key: keyof EvaluatorCriteria) => {
    updateActiveBlock((b) => {
      const current = b.evaluatorCriteria || {
        absencePosidoniaOrHabitats: false,
        immediateRemoval: false,
        scheduledRemoval: false,
        neutralizeAndMaintain: false,
      ...
      if (key === 'absencePosidoniaOrHabitats') {
        ...
        // Action is mutually exclusive among the 4 action verdicts
      ...
      return {
        ...b,
        evaluatorCriteria: next,
      };
    });
  };

  const activeSeabedTypes: SeabedTypeOption[] = rawActiveBlock.seabedTypes || [];
  const activePosidoniaDistances: PosidoniaDistanceOption[] = rawActiveBlock.posidoniaDistances || [];

  const toggleActiveSeabedType = (type: SeabedTypeOption) => {
    updateActiveBlock((b) => {
      const current = b.seabedTypes || [];
  ...
  const toggleActivePosidoniaDistance = (distance: PosidoniaDistanceOption) => {
    updateActiveBlock((b) => {
      const current = b.posidoniaDistances || [];
      const next = current.includes(distance)
        ? current.filter((d) => d !== distance)
        : [...current, distance];
      return {
        ...b,
        posidoniaDistances: next,
      };
    });
  };
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="bg-white rounded-3xl p-6 sm:p-7 shadow-xs border border-[#D1D1C7] space-y-5">
            <div className="flex items-center justify-between border-b border-[#E5E5DF] pb-3">
              ...
              <span className="text-xs font-mono text-[#64746B] uppercase tracking-wider">v3.0 • Campanya Subaquàtica</span>
            </div>
            ...
            {/* Caracterització del Fons i Proximitat a Posidònia (Opcional - Només es mostra a l'acta si es marca) */}
            <div className="pt-4 border-t border-[#E5E5DF] space-y-3">
              <div className="flex items-center justify-between flex-wrap gap-2">
                <label className="text-xs font-bold text-[#134E4A] flex items-center gap-1.5">
                  ...
                  Caracterització del Fons i Proximitat a Posidònia
                </label>
                <span className="text-[11px] text-[#64746B] italic">
                  (Caselles marcables multiselecció • Només es mostraran a la Fitxa d'Inspecció Oficial si estan marcades)
                </span>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Tipus de Fons */}
                <div className="bg-[#FAF9F6] border border-[#D1D1C7] rounded-xl p-3 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-[#134E4A] uppercase tracking-wider">
                      Tipus de Fons:
                    </span>
                    {activeSeabedTypes.length > 0 && (
                      <button
                        type="button"
                        onClick={() => updateActiveBlock((b) => ({ ...b, seabedTypes: [] }))}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {SEABED_TYPE_OPTIONS.map((opt) => {
                      const isChecked = activeSeabedTypes.includes(opt.id);
                      return (
                        <div
                          key={opt.id}
                          onClick={() => toggleActiveSeabedType(opt.id)}
                          className={`p-2 rounded-lg border cursor-pointer transition flex items-center gap-2 select-none ${
                            isChecked
                              ...
                              : 'bg-white border-[#D1D1C7] text-[#4A5D52] hover:bg-[#F3F4EE]'
                          }`}
                        >
                          <div
                ...
                <div className="bg-[#FAF9F6] border border-[#D1D1C7] rounded-xl p-3 space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    {activePosidoniaDistances.length > 0 && (
                      <button
                        type="button"
                        onClick={() => updateActiveBlock((b) => ({ ...b, posidoniaDistances: [] }))}
                        className="text-[10px] text-[#64746B] hover:text-[#134E4A] hover:underline"
                      >
                        Desmarcar
                      </button>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {POSIDONIA_DISTANCE_OPTIONS.map((opt) => {
                      const isChecked = activePosidoniaDistances.includes(opt.id);
                      return (
                        <div
                          key={opt.id}
                          onClick={() => toggleActivePosidoniaDistance(opt.id)}
                          className={`p-2 rounded-lg border cursor-pointer transition flex items-center gap-2 select-none ${
                            isChecked
                              ? 'bg-teal-100/70 border-teal-600 text-teal-950 font-bold shadow-xs'
                              : 'bg-white border-[#D1D1C7] text-[#4A5D52] hover:bg-[#F3F4EE]'
                          }`}
                        >
                          <div
          ...
          {presenceStatus === 'located' && (
            <div className="space-y-6">
                    ...
                    <div className="space-y-1 w-full">
                      <div className="flex items-center justify-between">
                        <span className="text-xs sm:text-sm font-bold text-[#134E4A]">
                          Recobriment &gt;10% o exemplars reproductors d'espècies protegides
                        </span>
              ...
              <div className="bg-white rounded-3xl p-6 sm:p-7 shadow-xs border border-[#D1D1C7] space-y-4">
                ...
                {activeC2Substrate === 'active_erosion_halo' && (
                  <div className="p-3.5 bg-[#FAF9F6] rounded-2xl border border-[#D1D1C7] flex flex-wrap items-center gap-4 text-xs">
                    <div className="flex items-center gap-2">
                      <span className="font-semibold text-[#134E4A]">Superfície estimada d'abrasió:</span>
                      <input
                        type="number"
                        step="0.5"
                        min="0.1"
                        max="500"
                        value={activeC2AbrasionArea}
                        onChange={(e) => {
                          const val = Number(e.target.value);
                          updateActiveBlockField('c2_abrasionAreaM2', val);
                          updateActiveBlockField('c2_haloRadiusM', val);
                        }}
                        className="w-24 px-2.5 py-1 border border-[#D1D1C7] rounded-lg bg-white font-bold text-[#134E4A] text-xs font-mono"
                      />
                      <span className="text-[#64746B] font-medium">metres quadrats (m²)</span>
                    </div>
                  ...
                  <div className="bg-[#FAF9F6] border border-[#D1D1C7] rounded-2xl p-4 space-y-2 text-xs">
                    ...
                    <p className="text-[#64746B] text-[11px] leading-relaxed">
                      {hydroAssessment.slidingRiskScore >= 3
                        ? 'Risc Alt (Blau): Estructura de pes reduït en zona somera, onades habituals superen la velocitat crítica.'
                        : hydroAssessment.slidingRiskScore === 2
                        ? 'Risc Mitjà-Alt (Verd): Pes moderat, mobilització en temporals forts.'
                        : hydroAssessment.slidingRiskScore === 1
                        ? 'Risc Baix (Taronja): Risc baix, mobilitat només en temporals extrems.'
                        : 'Absència de risc (Vermell): Estructura totalment estable per gran inèrcia o fondària segura.'}
                    </p>
                  </div>
                ) : (
              ...
              <div className="bg-white rounded-3xl p-6 sm:p-7 shadow-xs border border-[#D1D1C7] space-y-4">
                <div className="flex items-start justify-between gap-3 border-b border-[#E5E5DF] pb-3">
                  <div>
                    ...
                    <p className="text-xs text-[#64746B] mt-1.5 leading-relaxed">
                      Justificació ecològica: Grau d'integració física en el fons marí i afectació al substrat i mata en cas d'extracció.
                    </p>
                  </div>
                ...
                <div className="space-y-2.5">
                  ...
                  >
                    ...
                    <div className="space-y-1 w-full">
                      <div className="flex items-center justify-between">
                        <span className="text-xs sm:text-sm font-bold text-[#134E4A]">
                          Bloc lliure, no toca posidònia, no genera espai buit
                        </span>
                      ...
                      <p className="text-xs text-[#64746B]">
                        Bloc lliure sobre substrat mòbil o roca nua sense arrelament. Extracció directa sense dany.
                      </p>
                    </div>
                  </label>
                  ...
                  <label
                    className={`flex items-start gap-3 p-4 rounded-2xl border cursor-pointer transition ${
                      activeC4Stability === 'partial_burial_no_posidonia'
                        ? 'border-[#134E4A] bg-[#FAF9F6] text-[#134E4A] shadow-xs ring-1 ring-[#134E4A]'
                        : 'border-[#E5E5DF] hover:border-[#D1D1C7] text-[#4A4A43] bg-white'
                    }`}
                  >
                    <input
                      type="radio"
                      name={`c4_stability_${activeBlockIndex}`}
                      checked={activeC4Stability === 'partial_burial_no_posidonia'}
                      onChange={() => updateActiveBlockField('c4_stabilityIntegration', 'partial_burial_no_posidonia')}
                      className="mt-1 accent-[#134E4A]"
                    />
                    <div className="space-y-1 w-full">
                      <div className="flex items-center justify-between">
                        <span className="text-xs sm:text-sm font-bold text-[#134E4A]">
                          Enfonsament parcial, no toca posidònia, no genera espai buit
                        </span>
                        <span className="text-xs font-mono font-bold text-amber-900 bg-amber-100 px-2.5 py-0.5 rounded-md">
                          +4 punts
                        </span>
                      ...
                      <p className="text-xs text-[#64746B]">
                        Parcialment enfonsat en sediment sorrenc/llotós sense contacte amb mata viva ni creació de marmites.
                      </p>
                    </div>
                  </label>
                  ...
                  >
                    ...
                    <div className="space-y-1 w-full">
                      <div className="flex items-center justify-between">
                        <span className="text-xs sm:text-sm font-bold text-[#134E4A]">
                          Bloc no enterrat, sí genera un espai buit nou
                        </span>
                        <span className="text-xs font-mono font-bold text-emerald-900 bg-emerald-100 px-2.5 py-0.5 rounded-md">
                          -6 punts
                        </span>
                      ...
                      <p className="text-xs text-[#64746B]">
                        La remoció mecànica deixaria una depressió inestable o alteraria la dinàmica de sedimentació local.
                      </p>
                    </div>
                  </label>
                  ...
                  >
                    ...
                    <div className="space-y-1 w-full">
                      <div className="flex items-center justify-between">
                        <span className="text-xs sm:text-sm font-bold text-[#134E4A]">
                          Bloc fixat per les pròpies arrels de Posidònia
                        </span>
                        <span className="text-xs font-mono font-bold text-emerald-900 bg-emerald-100 px-2.5 py-0.5 rounded-md">
                          -12 punts
                        </span>
                      ...
                      <p className="text-xs text-[#64746B]">
                        El rizoma ha crescut adherit o envoltant el bloc (mata cohesionada). La retirada arrencaria el teixit vegetal viu.
                      </p>
                    </div>
                  </label>
                </div>
              ...
              {/* CRITERI DE L'AVALUADOR / SECCIÓ DE QUADRATS CLICABLES */}
              <div className="bg-white rounded-3xl p-6 sm:p-7 shadow-xs border-2 border-emerald-900/30 space-y-4">
                <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[#D1D1C7] pb-3">
                  <div className="flex items-center gap-2.5">
                    <div className="w-7 h-7 rounded-xl bg-[#134E4A] text-white flex items-center justify-center shadow-xs">
                      <UserCheck className="w-4 h-4" />
                    </div>
                    <div>
                      <h3 className="text-sm font-bold text-[#134E4A] flex items-center gap-2">
                        Critèri de l'Avaluador ({activeBlock.label || `Mort ${activeBlockIndex + 1}`})
                        {activeBlockEvaluatorCriteriaCount > 0 && (
                          <span className="text-[10px] font-mono font-bold bg-emerald-100 text-emerald-900 px-2 py-0.5 rounded-full border border-emerald-300">
                            {activeBlockEvaluatorCriteriaCount} actiu
                          </span>
                        )}
                      </h3>
                      <p className="text-xs text-[#64746B]">
                        Opcions de camp del tècnic per complementar el diagnòstic automàtic de la matriu.
                      </p>
                    </div>
                  </div>
                  <span className="text-[10px] font-mono text-[#64746B] bg-[#FAF9F6] px-2.5 py-1 rounded-lg border border-[#E5E5DF]">
                    Per defecte: No marcades
                  </span>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-1">
                  {/* Opció 1: Absència de posidònia/hàbitats protegits */}
                  <div
                    onClick={() => toggleActiveEvaluatorCriteria('absencePosidoniaOrHabitats')}
                    className={`p-3.5 rounded-2xl border-2 cursor-pointer transition flex items-start gap-3 select-none ${
                      activeEvaluatorCriteria?.absencePosidoniaOrHabitats
                        ? 'bg-emerald-50/80 border-emerald-600 shadow-xs'
                        : 'bg-[#FAF9F6] border-[#D1D1C7] hover:border-[#134E4A]/50 hover:bg-white'
                    }`}
                  >
                    ...
                    <div className="space-y-0.5">
                      <div className="text-xs sm:text-sm font-bold text-[#134E4A]">
                        Absència de posidònia/hàbitats protegits
                      </div>
                      <p className="text-[11px] text-[#64746B] leading-relaxed">
                        Constatació directa per part de l'avaluador de la manca de Posidònia o hàbitats protegits.
                      </p>
                    </div>
                  </div>

                  {/* Opció 2: Retirar mort de forma immediata */}
                  <div
                    onClick={() => toggleActiveEvaluatorCriteria('immediateRemoval')}
                    className={`p-3.5 rounded-2xl border-2 cursor-pointer transition flex items-start gap-3 select-none ${
                      activeEvaluatorCriteria?.immediateRemoval
                        ? 'bg-rose-50/90 border-rose-600 shadow-xs'
                        : 'bg-[#FAF9F6] border-[#D1D1C7] hover:border-rose-400 hover:bg-white'
                    }`}
                  >
                    <div className={`mt-0.5 w-5 h-5 rounded-md border flex items-center justify-center shrink-0 transition ${
                      activeEvaluatorCriteria?.immediateRemoval
                    ...
                    <div className="space-y-0.5">
                      ...
                      <p className="text-[11px] text-[#64746B] leading-relaxed">
                        Extracció urgent dictaminada per perill a la navegació o impacte greu in situ.
                      </p>
                    </div>
                  </div>

                  {/* Opció 3: Retirar mort de forma programada */}
                  <div
                    onClick={() => toggleActiveEvaluatorCriteria('scheduledRemoval')}
                    className={`p-3.5 rounded-2xl border-2 cursor-pointer transition flex items-start gap-3 select-none ${
                      activeEvaluatorCriteria?.scheduledRemoval
                        ? 'bg-amber-50/90 border-amber-600 shadow-xs'
                        : 'bg-[#FAF9F6] border-[#D1D1C7] hover:border-amber-400 hover:bg-white'
                    }`}
                  >
                    <div className={`mt-0.5 w-5 h-5 rounded-md border flex items-center justify-center shrink-0 transition ${
                      activeEvaluatorCriteria?.scheduledRemoval
                    ...
                    <div className="space-y-0.5">
                      ...
                      <p className="text-[11px] text-[#64746B] leading-relaxed">
                        Retirada planificada en campanya regular de sanejament del fons.
                      </p>
                    </div>
                  </div>

                  {/* Opció 4: Neutralitzar mort i mantenir */}
                  <div
                    onClick={() => toggleActiveEvaluatorCriteria('neutralizeAndMaintain')}
                    className={`p-3.5 rounded-2xl border-2 cursor-pointer transition flex items-start gap-3 select-none ${
                      activeEvaluatorCriteria?.neutralizeAndMaintain
                        ? 'bg-sky-50/90 border-sky-600 shadow-xs'
                        : 'bg-[#FAF9F6] border-[#D1D1C7] hover:border-sky-400 hover:bg-white'
                    }`}
                  >
                    <div className={`mt-0.5 w-5 h-5 rounded-md border flex items-center justify-center shrink-0 transition ${
                      activeEvaluatorCriteria?.neutralizeAndMaintain
                    ...
                    <div className="space-y-0.5">
                      <div className="text-xs sm:text-sm font-bold text-sky-950">
                        Neutralitzar mort i mantenir, a criteri de l'avaluador
                      </div>
                      <p className="text-[11px] text-[#64746B] leading-relaxed">
                        Tallar cadenes/grillets i conservar el bloc com a refugi subaquàtic.
                      </p>
                    </div>
                  </div>

                  {/* Opció 5: No retirar mort */}
                  <div
                    onClick={() => toggleActiveEvaluatorCriteria('noRemoval')}
                    className={`p-3.5 rounded-2xl border-2 cursor-pointer transition flex items-start gap-3 select-none md:col-span-2 ${
                      activeEvaluatorCriteria?.noRemoval
                        ? 'bg-teal-50/90 border-teal-600 shadow-xs'
                        : 'bg-[#FAF9F6] border-[#D1D1C7] hover:border-teal-400 hover:bg-white'
                    }`}
                  >
                    <div className={`mt-0.5 w-5 h-5 rounded-md border flex items-center justify-center shrink-0 transition ${
                      activeEvaluatorCriteria?.noRemoval
                    ...
                    <div className="space-y-0.5">
                      ...
                      <p className="text-[11px] text-[#64746B] leading-relaxed">
                        Preservació íntegra in situ com a escull consolidat de biodiversitat marina.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                {activeBlockEvaluatorCriteriaCount > 0 && (
                  <div className="p-3 bg-emerald-50 rounded-2xl border border-emerald-200 text-xs text-emerald-900 flex items-center gap-2">
                    ...
                    <span>
                      <strong>Criteri registrat:</strong> Apareixerà al diagnòstic i a la fitxa d'inspecció complementant el dictamen automàtic de la matriu de 128 casuístiques.
                    </span>
                  </div>
                )}
              </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="sticky top-24 space-y-6">
            ...
            <DecisionCard
              ...
              evaluatorCriteria={activeEvaluatorCriteria}
