...
/**
 * Estimates critical bottom orbital velocity ub and required wave height H for a block at depth d
 */
export function assessHydrodynamics(
  ...
  const { volumeM3, weightAirKg, submergedWeightKg } = calculateSubmergedWeight(
    dims.lengthCm,
    dims.widthCm,
    dims.heightCm,
  ...
  // ub ≈ sqrt( (2 * mu * W_sub) / (rho * Cd * A_frontal + Cl * A_top) )
  const heightM = dims.heightCm / 100;
  const lengthM = dims.lengthCm / 100;
