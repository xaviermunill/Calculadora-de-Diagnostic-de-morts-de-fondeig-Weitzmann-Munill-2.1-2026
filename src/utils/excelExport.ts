...
import { MortEvaluationRecord, getActiveEvaluatorCriteriaLabels } from '../types';
...
export function exportInventoryToExcel(records: MortEvaluationRecord[], filename?: string) {
  ...
  const inventoryRows = records.map((r, index) => {
    ...
    const criteriaLabels = r.evaluatorCriteria ? getActiveEvaluatorCriteriaLabels(r.evaluatorCriteria) : [];
    ...
    return {
      ...
      'Criteri Avaluador': criteriaLabels.length > 0 ? criteriaLabels.join('; ') : 'Sense criteri',
  ...
  records.forEach((r, rIdx) => {
    ...
    blocksList.forEach((b, bIdx) => {
      ...
      blockDetailRows.push({
        ...
        'C2 Superfície d\'Abrasió (m²)': b.c2_abrasionAreaM2 ?? b.c2_haloRadiusM ?? '',
