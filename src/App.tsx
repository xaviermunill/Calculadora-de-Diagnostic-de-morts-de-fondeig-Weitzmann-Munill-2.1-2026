...
export default function App() {
  ...
  const [batchDossierSelectedIds, setBatchDossierSelectedIds] = useState<string[] | undefined>(undefined);
  ...
  const handleOpenBatchDossier = (cala?: string, specificRecordIds?: string[]) => {
    setBatchDossierCalaFilter(cala || 'all');
    setBatchDossierSelectedIds(specificRecordIds);
  ...
  // Handler: Update multiple records (from data table editor)
  const handleUpdateMultipleRecords = (updatedList: MortEvaluationRecord[]) => {
    setRecords((prev) => {
      const updatedMap = new Map(updatedList.map((r) => [r.id, r]));
      const updatedPrev = prev.map((r) => updatedMap.get(r.id) || r);
      const existingIds = new Set(prev.map((r) => r.id));
      const newItems = updatedList.filter((r) => !existingIds.has(r.id));
      const finalRecords = [...newItems, ...updatedPrev];
      if (driveAutoSync && isDriveConnected) {
        pushRecordsToDrive(finalRecords);
      }
      return finalRecords;
    });
  };
    ...
    <div className="min-h-screen bg-[#F5F5F0] text-[#134E4A] flex flex-col font-sans antialiased selection:bg-[#134E4A] selection:text-[#FAF9F6]">
      ...
      {isBatchDossierOpen && (
        <BatchInspectionDossierModal
          ...
          initialSelectedIds={batchDossierSelectedIds}
          onClose={() => {
            setIsBatchDossierOpen(false);
            setBatchDossierSelectedIds(undefined);
          }}
        />
      )}
    </div>
