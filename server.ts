...
// Permanent Server-Side Inventory File Storage (Auto-sync without login or codes)
import fs from "fs";
...
// GET /api/inventory - Retrieve permanent inventory records
...
// POST /api/inventory - Permanently save and synchronize inventory records
app.post("/api/inventory", (req, res) => {
  try {
    ensureDataDirectory();
    const { records, lastUpdatedBy } = req.body;
    ...
    console.log(`[STORAGE SYNC] S'han desat permanentment ${records.length} registres a ${INVENTORY_FILE}`);
    ...
    return res.json({
      ...
      message: "Dades sincronitzades permanentment amb la carpeta de destí.",
    });
  } catch (error: any) {
